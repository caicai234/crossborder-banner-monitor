...
function computeImageHash(buffer) {
  // 方法1: 尝试用 sharp 做感知哈希（更抗噪）
  try {
    const sharp = require('sharp');
    // 如果 sharp 可用就用感知哈希
    return computePerceptualHash(buffer, sharp);
  } catch (e) {
    // sharp 不可用，使用纯 JS 方案
  }

  // 方法2: 纯 JS 感知哈希（手动解析 JPEG 像素）
  try {
    return computePureJSHash(buffer);
  } catch (e) {
    // 方法3: SHA-256 fallback
    return crypto.createHash('sha256').update(buffer).digest('hex').slice(0, 64);
  }
}

function computePerceptualHash(buffer, sharp) {
  // 同步版本：用 sharp 缩放到16x16灰度图
  const { data } = sharp(buffer)
    .resize(16, 16, { fit: 'fill' })
    .greyscale()
    .raw()
    .toBufferSync ? sharp(buffer).resize(16, 16, { fit: 'fill' }).greyscale().raw().toBufferSync()
    : { data: null };

  // sharp v0.33+ 可能是异步的，需要特殊处理
  throw new Error('use pure JS'); // 强制走纯JS
}

function computePureJSHash(buffer) {
  // 纯 JS 方案：取 JPEG 文件的前16KB数据做缩略哈希
  // 实际用buffer的特征采样：取等间距256个字节
  ...
  // 补到256位
  while (hash.length < 256) hash += hash;
  return hash.slice(0, 256);
}
